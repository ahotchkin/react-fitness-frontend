...
const Meals = props => {
  ...
  const mealCards = props.meals.filter(meal => props.diaryId === meal.relationships.diary.data.id).map(filteredMeal => <MealCard meal={filteredMeal} key={filteredMeal.id} diaryId={props.diaryId} diaryDate={props.diaryDate} foods={filteredMeal.attributes.foods} />)
